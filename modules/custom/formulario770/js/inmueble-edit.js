...
function content($scope, $location, $http) {
  console.log('santi inmueble edit js');
  ...
  $http.get($scope.urlUser).success(function (response) {
    ...
    /*if($scope.rol == 'Funcionario'){
      ...
      $scope.disabledF = true; //not disabled
    }*/
  });
  ...
  $scope.urlUser = $scope.my_location + "/usuariosRest";
  $http.get($scope.urlUser).success(function (response) {
    $scope.usuarioCurrent = response;
    console.log('usuario');
    console.log($scope.usuarioCurrent);
    if($scope.usuarioCurrent[0].field_user_nombre != ""){
      $scope.user_name = $scope.usuarioCurrent[0].field_user_nombre;
    }
    else{
      $scope.user_name = $scope.usuarioCurrent[0].field_user_razon_social;
    }
    $scope.urol_current = $scope.usuarioCurrent[0].roles_target_id;
    $scope.user_nameRol = $scope.user_name + ", " + $scope.urol_current;
  ...
  //INIT
  $scope.init_fechaTransmicion = function(fecha_transmicion){ //01-12-2016
  ...
  $http.get($scope.url).success(function (response) {
    ...
    console.log('init name_gradoParentesco');
    console.log($scope.name_gradoParentesco);
    ...
    */
    ...
    //$scope.baseImponible`Selected($scope.get_base_imponible);

    //base imponible actualizada:
    $scope.get_base_imponible_update = parseFloat($scope.inmuebles[0]['base_imponible_update']); //For HTML.

    //impuesto a pagar:
    $scope.get_impuesto = parseFloat($scope.inmuebles[0]['impuesto']); //for table HTML
    ...
    var fecha_a_pagarMulta = $scope.inmuebles[0]['fecha_a_pagarMulta'];  //10-12-2016
    ...
    var userRolC = $scope.inmuebles[0]['roles_target_id'];// user contribuyente
    //console.log("qqqqqqqqqqqqq" + $scope.urol_current);
    if($scope.urol_current == "Funcionario"){  //AQUI ESTAMOS TRABAJANDO SANTI************************
      $scope.show_depositoBancario = true;
      $scope.show_estadoForm = true;
      //$scope.show_stock = true;
      console.log("FUNCIONARIO");

      //STOCK
      /*if($scope.inmuebles[0]['stock'] == ""){ //si esta vacio
        var radiobtn = document.getElementById("stockInmueble");
        radiobtn.checked = true;  //checked true default html
        $scope.stock_value = "Si";  //value default save BD
      }
      else { //si no esta vacio
        var stock = $scope.inmuebles[0]['stock'];
        //console.log("stock:");
        //console.log(stock);
        if(stock == "Si"){
          var radiobtn = document.getElementById("stockInmueble");
          ...
          var radiobtn = document.getElementById("stockInmueble");
          radiobtn.checked = false;  //checked true default html
          $scope.stock_value = "No";  //value default save BD
        }
      }//fin stock*/

      if(diasTranscurridos > 0){ //muestra u oculta el campo "fecha a pagar la multa" si corresponde.
      swal('tiene multa');
      console.log("TIENE MULTA");
      //$scope.show_deposito = true;
      $scope.show_fechaPagoMulta = true;
      $scope.show_tablaMulta = true;
      }
      else{
        swal('no tiene multa');
        console.log("NO TIENE MULTA");
        /*$scope.show_fechaPagoMulta = true;
        $scope.show_tablaMulta = true;
        $scope.show_fechaPagoMulta = true;*/
    ...
    //if(userRolC == "Contribuyente"){
    if($scope.urol_current == "Contribuyente"){
      console.log("CONTRIBUYENTE");
      $scope.show_depositoBancario = false;
      $scope.show_estadoForm = false;
      //$scope.show_stock = false;
    ...
    if($scope.inmuebles[0]['m_mantenimiento_valor'] != ''){ //distinto de vacio
  ...
  //stock
  /*$scope.stock_init = function(stock_value) {
    console.log(stock_value);
    var radiobtn = document.getElementById("stockInmueble");
    radiobtn.checked = stock_value;  //checked true default html
    if(stock_value == true){
      $scope.stock_value = "Si";  //value default save BD
    }
    else{
  ...
  $scope.init_gradoParenteso = function (name_gradoParentesco){
    ...
    $http.get($scope.url).success(function (response) {
      ...
      console.log('dentro init gradoParentescoNombre');
    ...
    $http.get($scope.url).success(function(respuesta) {
      ...
      if($scope.get_categoria != undefined || $scope.grado_parentesco != undefined || $scope.datoLista.length != 0 || $scope.dataList.length != 0){
        ...
        if(!$scope.dateTransmicion[0]){
          swal('Atención', 'La fecha Ingresada no tiene UFV. Porfavor ingrese otra fecha', 'info');
  ...
  function get_ufv_ma(convertDateDiaT, convertDateMesT, convertDateAnoT){
    ...
    $http.get($scope.url).success(function(respuesta) {
      ...
      if(!$scope.ufv[0]){
        swal('Atención', 'La fecha Ingresada no tiene UFV. Porfavor ingresar otra fecha', 'info');
        $scope.fecha_transmicion = '';
        $scope.get_base_imponible = '';
        $scope.get_base_imponible_update  = '';
        $scope.get_impuesto  = '';
      ...
      switch($scope.feriados.length) {
        ...
        case 0:
          ...
          if(date_convert2.getDay() == 0 || date_convert2.getDay() == 6){
            ...
            //swal('Atención', "Usted tiene que pagar el IMPUESTO hasta la fecha: " + dateFormatedDMA + ".  Caso contrario se le cargara una multa economica.", 'info');
            $scope.convertMes($scope.convertDateMesT2[0]);
          }
          break;
        case 1: //la fecha de pago ha caido en un feriado
          ...
          $http.get($scope.url).success(function(respuesta) {
            ...
            if($scope.feriados.length == 0){
              ...
              //swal('Atención', "Usted tiene que pagar el IMPUESTO hasta la fecha: " + $scope.convertDateDiaT2 + "-" + $scope.convertDateMesT2 + "-" + $scope.convertDateAnoT2 + ".  Caso contrario se le recargara una multa economica.", 'info');
              $scope.convertMes($scope.convertDateMesT2[0]);
              $scope.get_fecha_pago = date_convert2;
            }
  ...
  //var array_multa_total = [];
  $scope.dateSelected = function(get_fecha_pago_multa){ //Sat Dec 10 2016 00:00:00 GMT-0400 (BOT)
    if(get_fecha_pago_multa != null){ //si tiene valor: get_fecha_pago_multa
      //console.log('---- dateSelected: get_fecha_pago_multa');
      ...
      //console.log('$scope.datePagoM: ' + $scope.datePagoM);
      ...
      //console.log('---- dateSelected timestamp: ' + $scope.fechaPagarMulta);
      //console.log($scope.fechaPagarMulta);
      ...
      $http.get($scope.url).success(function(respuesta) {
        ...
        if(!$scope.ufv[0]){
          //santi working :
          swal('Atención', 'La fecha Ingresada para pagar la Multa no tiene UFV. Porfavor ingresar otra fecha', 'info');
          $scope.get_fecha_pago_multa = '';
          $scope.multa_mantenimientoAlValor ='';
          $scope.multa_tasaDeInteres ='';
          $scope.multa_alIncumplimiento = '';
          $scope.multaTotal = '';
        }
        else{
          //la fecha ingresada tiene ufv:
          $scope.ufv_fecha_aPagar = $scope.ufv[0].field_uc_valor;
          //console.log($scope.ufv_fecha_aPagar);
          $scope.url = $scope.my_location + "/inmuebleRest/" + $scope.urlInmuebleID;
          $http.get($scope.url).success(function (response) {
            $scope.inmuebles = response;
            $scope.impuestoPago = $scope.inmuebles[0]['impuesto'];
            ...
            $http.get($scope.url).success(function(respuesta) {
              ...
              if(!$scope.ufv_fechaPago[0]){
                swal('Atención', 'La fecha el cual se vence el Pago del Impuesto no tiene UFV.', 'info');
              }
              else{
  ...
  $scope.get_mantValor = function(ufv_fecha_aPagar, impuestoPago, ufv_fechaPago, ti_fecha_aPagar, datePagoM){
    ...
    //$scope.multa_icumplimiento(ufv_fecha_aPagar); //function  WORKING-------------------
    ...
    $scope.get_tasaInteres($scope.resultadoMV, impuestoPago, ti_fecha_aPagar, datePagoM, ufv_fecha_aPagar); //WORKING
  };

  //Get Tasa de Interes.
  //$scope.get_tasaInteres = function(resultadoMV, impuestoPago, ti_fecha_aPagar, datePagoM){
  $scope.get_tasaInteres = function(resultadoMV, impuestoPago, ti_fecha_aPagar, datePagoM, ufv_fecha_aPagar){
    ...
    $http.get($scope.url).success(function (response) {
      ...
      $http.get($scope.url).success(function (response) {
        ...
        //console.log('xxxxx multa_tasaDeInteres');
        //console.log($scope.multa_tasaDeInteres);

        $scope.multa_icumplimiento(ufv_fecha_aPagar, $scope.multa_tasaDeInteres); //WORKING
        //array_multa_total.push($scope.multa_tasaDeInteres);
      });
    });
    //Formular Original:
  ...
  $scope.multa_icumplimiento = function(ufv_fecha_aPagar, multa_tasaDeInteres){
    $scope.url = $scope.my_location + "/inmuebleRest/" + $scope.urlInmuebleID;
    $http.get($scope.url).success(function (response) {
      $scope.inmuebles = response;
      ...
      $http.get($scope.url).success(function (response) {
        ...
        /*//console.log('11111 get_impuesto');
        //console.log($scope.get_impuesto);
        //console.log('22222 multa_mantenimientoAlValor');
        //console.log($scope.multa_mantenimientoAlValor);
        //console.log('333333 multa_tasaDeInteres');
        //console.log(multa_tasaDeInteres);
        //console.log('44444 multa_alIncumplimiento');
        //console.log($scope.multa_alIncumplimiento);*/


        $scope.multaTotal = $scope.get_impuesto + $scope.multa_mantenimientoAlValor + multa_tasaDeInteres + $scope.multa_alIncumplimiento;
        ...
        //$scope.multaTotal = $scope.get_impuesto + $scope.multa_mantenimientoAlValor + $scope.a + $scope.multa_alIncumplimiento;
        //console.log('aaaa multaTotal');
        //console.log($scope.multaTotal);
      });
    });
  };
  ...
  */
  ...
  $scope.add_cesionarios = function(){
    ...
    if($scope.sumPorcCedent <= 100){
      ...
      swal('Atención', "La suma de los porcentajes de los Cedentes tiene que ser menor o igual al 100 %.", "Porfavor elimina la fila exedente.", 'info');
  ...
  function completeArrayDeposito(cedeTam, cedeD, montoD, nombreD, fechaD){
    ...
    /*console.log('cedeTam: ' + cedeTam);
    console.log('cedeD: ' + cedeD);
    console.log('montoD: ' + montoD);
    console.log('nombreD: ' + nombreD);
    console.log('fechaD: ' + fechaD); //20/02/2017
    */
    $scope.totalDeposito = [];
  ...
  $scope.submit = function() {
    console.log('----submit----');
    //console.log('timestamp:' + $scope.fechaPagarMulta );
    //console.log('qqqqqqqqqqqq date_a_pagarMulta');
    //console.log($scope.date_a_pagarMulta);
    ...
    var inmueble = {
      ...
      'estado_form' :{ 'value': $scope.estado},
      'user_edit' : {'value': $scope.user_nameRol},//guardar user_current: name rol
      //'fecha_a_pagarMulta' :{ 'value': $scope.date_a_pagarMulta },
      'fecha_a_pagarMulta' :{ 'value': $scope.fechaPagarMulta},
      //'fecha_a_pagarMulta' :{ 'value': $scope.caballero },
      //'fecha_a_pagarMulta' :{ 'value': '1476244800' }, //OK
      'm_impuesto_pago' : { 'value': $scope.get_impuesto },
      'm_mantenimiento_valor' : { 'value': parseFloat($scope.multa_mantenimientoAlValor) },
      'm_tasa_interes' : { 'value': parseFloat($scope.multa_tasaDeInteres) },
      'm_multa_incumplimiento' : { 'value': parseFloat($scope.multa_alIncumplimiento) },
      'multa' :{ 'value': $scope.multaTotal},
      'nro_res_admin' :{ 'value': $scope.resAdmin},
      'formulario' :{ 'value': $scope.formulario},
      'nro_orden' :{ 'value': $scope.orden},
      'stock' :{ 'value': $scope.stock_value},
      //----FIN EDIT OK----
  ...
  $scope.editInmueble = function(i){
      //console.log('i');
      console.log(i);
      console.log("Entro a EDIT inmueble");
      return $http({
        ...
        //redirect to:
        setTimeout(function(){
          window.location = "/reporte-inmuebles";
        }, 2100);
  ...
  /*$scope.stockFunction = function() {
    document.getElementById("nro-orden").required;
    swal({
      title: "Atencion",
      text: "El campo: Numero de Orden es Requerido",
      type: "warning"
    });
  };*/
