...
function content($scope, $location, $http) {
  console.log('santi edit mueble js');
  ...
  $http.get($scope.urlUser).success(function (response) {
    ...
    if($scope.usuarioCurrent[0].field_user_nombre == ''){
      $scope.user_nameRol = $scope.usuarioCurrent[0].field_user_razon_social + ", " + $scope.urol_current;
    }
    else{
  ...
  $scope.dateSelected = function(get_fecha_pago_multa){
    if(get_fecha_pago_multa != null){
      ...
      $scope.ti_fecha_aPagar = $scope.fechaPagoM_ano + "/" + parseInt($scope.fechaPagoM_mes);
  ...
  $scope.get_tasaInteres = function(resultadoMV, impuestoPago, ti_fecha_aPagar, datePagoM, ufv_fecha_aPagar){
    console.log('ti_fecha_aPagar: ' + ti_fecha_aPagar);
  ...
  */
  ...
  $scope.submit = function() {
    ...
    var inmueble = {
      ...
      'status_form_750' :{ 'value': $scope.estado},
      ...
      'user_edit' : {'value': $scope.user_nameRol},
      //----FIN EDIT OK----
  ...
  $scope.editInmueble = function(i){
      //console.log('i');
      console.log("Entro a edit mueble");
      console.log(i);
      return $http({
        url: $scope.my_location + '/admin/structure/mueble/' + $scope.urlInmuebleID,
        method: 'PATCH',
        data: i,
        headers: {
